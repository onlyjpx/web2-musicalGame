...
export const buscarMusicasDeezerLista = async (musicaNome, artistaNome) => {
    ...
    const base = response.data.data.map((t, idx) => {
        ...
        return {
            ...
            duracao: t.duration,
            link: t.link,
            rank: t.rank, // popularidade Deezer (0..100000)
            position: idx,
            _matchScore: 0
        }
    });
    ...
    base.forEach(item => {
        ...
        if (titleLower === nomeLower) score += 60; // match exato forte
        ...
        if (termoArtista) {
            ...
            if (artLower === termoArtista) score += 35;
            else if (artLower.includes(termoArtista)) score += 15;
        }
        // proximidade de tamanho
        score += Math.max(0, 12 - Math.abs((item.titulo||'').length - nomeLower.length));
        // popularidade Deezer normalizada (rank 0..100000) - maior rank = mais popular
        const pop = (item.rank || 0) / 100000; // 0..1
        score += pop * 50; // peso forte para trazer as famosas ao topo
